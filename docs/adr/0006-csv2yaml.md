...
- `Public` が `TRUE` の行だけを出す。開始日が `2026-09-01` 以降のイベントに限る（`ruby csv2yaml.rb YYYY-MM-DD` または `make events SINCE=...` で変更可）。CSV の `開催地` 列は [ADR-0018](0018-event-region.md) のとおり 13 種のうち 1 つで、YAML の `region` にする（古い `tag` 列も読む）。
