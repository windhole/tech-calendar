...
- `Public` が `TRUE` の行だけを出す。開始日が `2026-09-01` 以降のイベントに限る（`ruby csv2yaml.rb YYYY-MM-DD` または `make events SINCE=...` で変更可）。CSV の `tag` 列は [ADR-0018](0018-event-region.md) のとおり開催地（13 種のうち 1 つ）で、YAML の `region` にする。
