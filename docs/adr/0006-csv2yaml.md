...
- `Public` が `TRUE` の行だけを出す。開始日が `2026-09-01` 以降のイベントに限る（`ruby csv2yaml.rb YYYY-MM-DD` または `make events SINCE=...` で変更可）。CSV の `tag` 列は [ADR-0017](0017-event-tags.md) のとおり、カンマ区切りを YAML の `tags` 配列にする。
