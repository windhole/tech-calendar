...
- イベント名にマウスを乗せると全文、クリックすると詳細ダイアログ（開催日・会場・タグ・「イベントサイトを開く」）
...
- ヘッダ下のタグチップで、カレンダー表示とリスト表示を同じ条件で絞り込む（複数選択は OR。`?tag=` が URL に残る）
...
| `location` | `string` | 会場 |
| `url` | `string` | 詳細 URL |
| `tags` | `string[]` | 任意。無いときはキーごと省略。古い `tag: 文字列` も 1 タグとして読む |
...
- `tag` は任意。`,` と `、` で複数に分割し、YAML の `tags` 配列にする。空ならキーを出さない
